    ...
    while True:
        ...
        if len(bounding_boxes) > 0:
            ...
            for i in range(0, len(bounding_boxes)):
                ...
                # det[0] = max(1, det[0])
                # det[1] = max(1, det[1])
                # det[2] = min(image.shape[1]-1, det[2])
                # det[3] = min(image.shape[0]-1, det[3])
                # start_time = time()
                ...
                if recog_data[i][0] == "Unknown":
                    cv2.rectangle(image, (int(det[0]), int(det[1])), (int(det[2]), int(det[3])), (0, 0, 255), 3)  # draw bounding box for the face
                    cv2.putText(image, recog_data[i][0] + '_' + str(recog_data[i][1]), (int(det[0]), int(det[1])),
                                cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 2, cv2.LINE_AA)
                else:
                    cv2.rectangle(image, (int(det[0]), int(det[1])), (int(det[2]), int(det[3])), (255, 200, 0), 3)  # draw bounding box for the face
                    cv2.putText(image, recog_data[i][0] + '_' + str(recog_data[i][1]), (int(det[0]), int(det[1])),
...
def findPeople(features_arr,dataset, people_names, thres=0.7, percent_thres=80):
    '''
        ...
        if len(bounding_boxes) == 0:
            ...
            for (i, det) in enumerate(bounding_boxes):
                ...
                if len(positions_arr) == 3:
                    ...
                    sys.exit(-1)
                cv2.rectangle(image, (det[0], det[1]), (det[2], det[3]), (0, 255, 255), 2)  # draw bounding box for the face
